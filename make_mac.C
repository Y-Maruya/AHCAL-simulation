...
int make_mac(){
    for( int i =0 ; i<1000 ;i++){
        std::ofstream ofs(Form("./mac/run_off_%d.mac",i));
        ofs<<"/control/verbose 2"<<std::endl;
        ofs<<"/run/verbose 1"<<std::endl;
        ofs<<"/event/verbose 0"<<std::endl;
        ofs<<"/tracking/verbose 0"<<std::endl;
        ofs<<"/generator/inputFileName /afs/cern.ch/user/y/ymaruya/private/FASERlink/genie_data/faser_off.n10000."<<i<<".gfaser.root"<<std::endl;
